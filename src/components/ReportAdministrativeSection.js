...
const ReportAdministrativeSection = ( { internship } ) => {
  ...
  const handleSendReport = async( values ) => {
    confirm( {
        title: '¿Confirmas que deseas marcar las horas de este informe como registradas en el SAEw?',
  ...
  return (
    <>
      <Descriptions title={ <Divider orientation='center'><strong>REPORTE DE LA MÁXIMA AUTORIDAD</strong></Divider> }
                    bordered column={ 2 }
                    extra={ currentUser.role === 'ROLE_ADMINISTRATIVE'
                    && internship.status === 'approved'
                    && <Button type='primary' onClick={ handleSendReport }>Marcar como horas registradas en el
                      SAEw</Button>
                    }>
