  ...
  return (
    <>
      <Descriptions title={ <Divider orientation='center'><strong>REPORTE DEL ESTUDIANTE</strong></Divider> }
        ...
        <Descriptions.Item label='Horas ejecutadas'>{ internship.hours_worked }</Descriptions.Item>
        <Descriptions.Item label='Actividades principales desarrolladas'>
          <ol>
            { internship.activities.map( ( activity, i ) => <li key={ i }>{ activity.description }</li> ) }
          </ol>
        </Descriptions.Item>
        <Descriptions.Item label='Observaciones del estudiante'>{ internship.student_observations }</Descriptions.Item>
        ...
        <Descriptions.Item label='Temáticas que hicieron falta y que no constan en la malla curricular'>
          <ul>
            { internship.recommended_topics.map( ( topic, i ) => <li key={ i }>{ topic.name }</li> ) }
          </ul>
        </Descriptions.Item>
      </Descriptions>

      <Modal
        title={ `${ internship.status === 'in_progress'
          ? 'Completar'
          : 'Editar' } reporte` }
        visible={ showModal }
        width={ 800 }
        ...
        onOk={ () => form.submit() }>
        <ReportStudentSectionForm form={ form } onSubmit={ handleSendReport } internship={ internship } />
      </Modal>
    </>
  );
};
