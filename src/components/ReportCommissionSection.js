  ...
  return (
    <>
      <Descriptions title={ <Divider orientation='center'><strong>REPORTE DE LA COMISIÓN DE PPP</strong></Divider> }
                    bordered column={ 2 }
                    extra={ currentUser.role === 'ROLE_COMMISSION'
                    && internship.status === 'commission_pending'
                    && <Button type='primary' onClick={ () => setShowModal( true ) }>Completar reporte</Button>
                    }>
        <Descriptions.Item label='Una vez revisada, analizada y validada la información reportada por el estudiante, ¿avala la aprobación de las horas de prácticas preprofesionales indicadas en este informe?'
                           span={ 2 }>
          {
        ...
        <Descriptions.Item label='Observaciones'
                           span={ 2 }>{ internship.commission_observations }</Descriptions.Item>
      </Descriptions>
      ...
      <Modal
        ...
        onOk={ () => form.submit() }>
        <ReportCommissionSectionForm form={ form } onSubmit={ handleSendReport } internship={ internship } />
      </Modal>
    </>
  );
};
